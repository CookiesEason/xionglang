package com.base.generic.service.impl;
 ...
 * @Time：2017年7月7日 @此类作用：处理所有的一般查询方法， 需要特殊查询可在子类重写父类的方法
...
public class GenericServiceImpl<T> {
	...
	public T create(GenericService<T> genericService, T entity, Object... params) {
	...
	public T update(GenericService<T> genericService, T entity, Object... params) {
		genericService.setGenericDao();
	...
	public boolean delete(ClassTypeInfoForm typeInfo) throws ConnectionException, SQLException {
		try {
			return DBSqlUtil.getInInstance().delete(typeInfo);
	...
	public boolean update(ClassTypeInfoForm typeInfo) throws ConnectionException, SQLException {
		try {
			return DBSqlUtil.getInInstance().update(typeInfo);
		} catch (SQLException e) {
			logger.error("错误行号:【" + new Throwable().getStackTrace()[0].getLineNumber() + "】" + e.getMessage());
			throw new SQLException(e.getMessage());
		}
	...
	public boolean add(ClassTypeInfoForm typeInfo) throws ConnectionException, SQLException {
		try {
			return DBSqlUtil.getInInstance().add(typeInfo);
		} catch (SQLException e) {
			logger.error("错误行号:【" + new Throwable().getStackTrace()[0].getLineNumber() + "】" + e.getMessage());
			throw new SQLException(e.getMessage());
		}
