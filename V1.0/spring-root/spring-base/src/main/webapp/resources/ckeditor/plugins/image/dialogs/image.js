...
(function() {
	var a = function(b, c) {
		...
		function m() {
			var B = arguments, C = this.getContentElement('advanced',
					'txtdlgGenStyle');
		...
		var p = function(B, C) {
			...
			if (C == 'check') {
				if (!B.userlockRatio && D.getCustomData('isReady') == 'true') {
					var E = B.getValueOf('info', 'txtWidth'), F = B.getValueOf(
							'info', 'txtHeight'), G = D.$.width * 1000
							/ D.$.height, H = E * 1000 / F;
					B.lockRatio = false;
					if (!E && !F)
						B.lockRatio = true;
			...
			if (CKEDITOR.env.hc) {
				var J = I.getChild(0);
				J.setHtml(B.lockRatio ? CKEDITOR.env.ie ? '■' : '▣'
						: CKEDITOR.env.ie ? '□' : '▢');
			}
			return B.lockRatio;
		}, q = function(B) {
			...
			if (C.getCustomData('isReady') == 'true') {
				var D = B.getContentElement('info', 'txtWidth'), E = B
			...
			;
			var E = this.getDialog(), F = '', G = this.id == 'txtWidth' ? 'width'
					: 'height', H = C.getAttribute(G);
			if (H)
				F = D(H, F);
			F = D(C.getStyle(G), F);
			this.setValue(F);
		...
		return {
			...
			onShow : function() {
				...
				H.firstLoad = true;
				H.addLink = false;
				var B = H.getParentEditor(), C = B.getSelection(), D = C
						&& C.getSelectedElement(), E = D && D.getAscendant('a');
				CKEDITOR.document.getById(y).setStyle('display', 'none');
										...
										children : [
												{
													...
													validate : CKEDITOR.dialog.validate
															.notEmpty(b.lang.image.urlMissing)
												},
												{
													type : 'button',
													id : 'browse',
																				...
																				C
																						.on(
																								...
																								function(
																										D) {
																									var I = this;
																									var E = p(I), F = I.originalElement, G = I
																											.getValueOf(
																													'info',
																													'txtWidth');
																									if (F
															...
															children : [
																	{
																		...
																		validate : CKEDITOR.dialog.validate
																				.integer(b.lang.image.validateBorder),
																		setup : function(
																				B,
																				C) {
																			if (B == d) {
																	...
																	},
																	{
																		...
																		validate : CKEDITOR.dialog.validate
																				.integer(b.lang.image.validateHSpace),
																		setup : function(
																				B,
																				C) {
																			if (B == d) {
																				var D, E, F, G = C
																						.getStyle('margin-left'), H = C
																	...
																	},
																	{
																		...
																		validate : CKEDITOR.dialog.validate
																				.integer(b.lang.image.validateVSpace),
																		setup : function(
																				B,
																				C) {
																			if (B == d) {
																				var D, E, F, G = C
																						.getStyle('margin-top'), H = C
																	...
																	},
																	{
																		...
																		items : [
																				[
																						...
																						'' ],
																				[
																						b.lang.common.alignLeft,
																						'left' ],
																				[
																						b.lang.common.alignRight,
																						'right' ] ],
																		onChange : function() {
																			l(this
																					.getDialog());
									...
									commit : function(B, C) {
										...
										if (B == e)
											if (E.getValue() || E.isChanged()) {
												...
												if (E.getValue()
														|| !b.config.image_removeLinkByEmptyURL)
					...
					},
					{
						...
						elements : [
											...
											},
											{
												...
												items : [
														[ b.lang.common.notSet,
																'' ],
														[
																b.lang.common.langDirLtr,
																'ltr' ],
														[
																b.lang.common.langDirRtl,
																'rtl' ] ],
												setup : function(B, C) {
													if (B == d)
														this
								...
								},
								{
									...
									validate : CKEDITOR.dialog.validate
											.inlineStyle(b.lang.common.invalidInlineStyle),
									'default' : '',
									setup : function(B, C) {
										if (B == d) {
											...
											if (!D && C.$.style.cssText)
												D = C.$.style.cssText;
											this.setValue(D);
											var E = C.$.style.height, F = C.$.style.width, G = (E ? E
													: '').match(h), H = (F ? F
													: '').match(h);
