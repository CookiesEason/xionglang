...
<form method="post" class="form" id="idAdminUserForm">
	<fieldset>
		...
		<table class="table"
			...
			<tr>
				<th>菜单名 <input type="hidden" value="${menu.id}" name="id" />
				</th>
				...
				<td><select id="fatherMenu" name="father.id"
					class="easyui-combobox">
						<c:forEach items="${menus}" var="m" varStatus="i">
							<option value="${m.id}"
								<c:if test='${m.id==menu.id}'>selected="selected"</c:if>>${m.name}</option>
						</c:forEach>
				</select></td>
			</tr>
			<tr>
				...
				<td><c:forEach items="${roles}" var="role" varStatus="i">
						<label> <input type="checkbox"
							...
							name="roles.id" value="${role.id}"> <span>${role.name}</span>
						</label>
					</c:forEach></td>
			</tr>
		</table>
