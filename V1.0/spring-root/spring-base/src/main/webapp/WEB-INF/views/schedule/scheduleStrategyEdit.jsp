...
<form id="scheduleStrategyForm" method="get" name="scheduleStrategyForm"
	...
	<input type="hidden" name="path"
		value="path=private.schedule.scheduleStrategyDeal"> <input
		type="hidden" name="action" value="<%=actionName%>" />
	<table>
		<tr>
			<td>策略名称:</td>
			<td><input type="text" id="strategyName" name="strategyName"
				<%=editSts%> value="<%=scheduleStrategy.getStrategyName()%>"
				width="30"></td>
			<td>必须填写，不能有中文和特殊字符</td>
		</tr>
		<tr>
			<td>任务类型:</td>
			<td><input type="text" id="kind" name="kind"
				value="<%=scheduleStrategy.getKind().toString()%>" width="30">
			</td>
		...
		<tr>
			...
			<td><input type="text" id="taskName" name="taskName"
				value="<%=scheduleStrategy.getTaskName()%>" width="30"></td>
		...
		<tr>
			<td>单JVM最大线程组数量:</td>
			<td><input type="text" name="numOfSingleServer"
		...
		<tr>
			<td>最大线程组数量：</td>
			<td><input type="text" name="assignNum"
				value="<%=scheduleStrategy.getAssignNum()%>" width="30"></td>
			<td>所有服务器总共运行的最大数量</td>
		</tr>
		<tr>
			<td>IP地址(逗号分隔)：</td>
			<td><input type="text" name="ips" value="<%=ips%>" width="30">
			</td>
