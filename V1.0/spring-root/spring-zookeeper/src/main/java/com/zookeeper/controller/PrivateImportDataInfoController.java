...
public class PrivateImportDataInfoController {
	...
	public String goImportDataInfo(HttpServletRequest request) {
		...
		try {
			ViewInfoForm viewInfoForm = new ViewInfoForm(searchParamList, "/private/importDataInfo", "idImportDataInfoForm", PrivateImportDataInfoController.OBJECTINFO);
	...
	public String viewTableInfo(@PathVariable("id") int id, HttpServletRequest request) {
		try {
			List<ImportDataColm> list = DBSqlUtil.getInInstance().getObjectList(new ClassTypeInfoForm(ImportDataColm.class, "SELECT ordeid,coluna,defava,comment FROM base_import_colm WHERE IID=" + id + " ORDER BY ordeid ASC "));
	...
	public JSON addTableInfo(@PathVariable("id") int id, ImportDataColm importDataColm, HttpServletRequest request) throws ConnectionException {
		...
		try {
			if (importDataColm.getImportDataColm() != null && id != 0) {
				...
				importDataColmService.delete(new ClassTypeInfoForm("DELETE FROM base_import_colm WHERE IID=" + id));
				if (com.zookeeper.util.DBSqlUtil.getInstance().insertBatch("base_import_colm", new InsertBatchForm(values, listColums))) {
					importDataColmService.update(new ClassTypeInfoForm("UPDATE ID_SEQUENCE SET VALUE=" + maxId + " WHERE TABLENAME='base_import_colm'"));
				} else {
					info = "批处理出错";
