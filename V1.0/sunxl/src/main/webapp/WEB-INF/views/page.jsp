...
<table class="page_turn">
	...
	<tr>
		<td>
			共计&nbsp;<font class="page_count">${pageSupport.rowCount }</font>&nbsp;条记录
		</td>
		<c:if test="${pageSupport.pageEnd > 1 }">
			...
			<c:if test="${pageSupport.pageNow > 1}">
				<td>
					<a class="next_page" href='javascript:;' onclick="pageTurning('${pageSupport.pageNow-1}')">上一页</a>
				</td>
			</c:if>
			<td>&nbsp;</td>
			...
			<c:forEach items="${pageSupport.displayPages}" var="v_paper">
				<td>
					<c:if test="${v_paper eq pageSupport.pageNow}" var="v_current">
						<a style="font-weight: bold;">${v_paper}</a>&nbsp;</c:if>
					<c:if test="${!v_current}">
			...
			<c:if test="${pageSupport.pageNow < pageSupport.pageEnd}">
				<td>
					<a class="next_page" href='javascript:;' onclick="pageTurning('${pageSupport.pageNow+1}')">下一页</a>
				</td>
			</c:if>
			<c:if test="${pageSupport.pageNow != pageSupport.pageEnd}">
				<td>
					<a class="next_page" href='javascript:;' onclick="pageTurning('${pageSupport.pageEnd}')">&nbsp;末页</a>
				</td>
			</c:if>
			...
			<c:if test="${pageSupport.selectPluginStarted}">
				...
				<td>
					跳至
					<select onchange="selectTurning(this)">
			...
			<c:if test="${pageSupport.textPluginStarted}">
				...
				<td>
					前往
					<input id="TEXT-PLUGIN" size='3' value="${pageSupport.pageNow }"
						onkeyup="value=value.replace(/[^\\d]/g,'')" />
					<input type="button" value="GO>>" onclick="turningText()">
				</td>
			</c:if>
