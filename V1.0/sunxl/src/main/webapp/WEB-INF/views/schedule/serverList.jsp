...
<table style="border: 0;">
	<tr>
		<td>任务类型：</td>
		<td>
			<input type="text" id="baseTaskType" value="<%=baseTaskType == null ? "" : baseTaskType%>">
		</td>
		<td>任务域：</td>
		<td>
			<input type="text" id="ownSign" value="<%=ownSign == null ? "" : ownSign%>">
		</td>
		<td>IP：</td>
		<td>
			<input type="text" id="ip" value="<%=ip == null ? "" : ip%>">
		</td>
		<td>排序：</td>
		<td>
			<input type="text" id="orderStr" value="<%=orderStr == null ? "" : orderStr%>">
		</td>
		<td>
			<input type="button" onclick="query()" value="查询" style="width: 100;">
		</td>
	</tr>
</table>
<%
...
<table id="list" border="1" style="border-COLLAPSE: collapse; display: block;">
	<tr>
		...
		<th>
			注册时间<BR />[REGISTER_TIME]
		</th>
		<th>
			心跳时间<BR />[HEARTBEAT_TIME]
		</th>
		<th>
			取数时间<BR />[LAST_FETCH_DATA_TIME]
		</th>
		<th nowrap>
			版本<BR />[VERSION]
		</th>
		<th nowrap>
			下次开始<BR />[NEXT_RUN_START_TIME]
		</th>
		<th nowrap>
			下次结束<BR />[NEXT_RUN_END_TIME]
		</th>
		<th>
			处理器<BR />[MANAGER_FACTORY]
		</th>
		<th>处理详情</th>
	</tr>
	<%
