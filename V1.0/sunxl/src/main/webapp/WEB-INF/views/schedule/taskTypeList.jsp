		...
		<%
			...
			for (int i = 0; i < taskTypes.size(); i++) {
				...
				if (strs != null) {
					...
					if (taskItems.length() >= 12) {
						taskItems = taskItems.substring(0, 11) + "....";
					}
				}
				taskParameters = taskTypes.get(i).getTaskParameter();
				if (taskParameters.length() >= 12) {
					taskParameters = taskParameters.substring(0, 11) + "....";
				}
		%>
		<tr
			onclick="openDetail(this,'<%=taskTypes.get(i).getBaseTaskType()%>')">
			<td><%=(i + 1)%></td>
			<%
				if ("true".equals(isManager)) {
			%>
			<td><div style="width: 100px; text-align: center;">
					<a target="taskDetail"
						href="taskTypeEdit.jsp?taskType=<%=taskTypes.get(i).getBaseTaskType()%>&csrftoken=${csrftoken}"
						style="color: #0000CD">编辑</a> <a target="taskDetail"
						href="taskTypeDeal.jsp?action=clearTaskType&taskType=<%=taskTypes.get(i).getBaseTaskType()%>&csrftoken=${csrftoken}"
						style="color: #0000CD">清理</a> <a target="taskDetail"
						href="javascript:void(0)"
						onclick="validateDel('<%=taskTypes.get(i).getBaseTaskType()%>')"
						style="color: #0000CD">删除</a>
				</div></td>
			<%
				}
			%>
...
%>
<a target="taskDetail"
	href="<%=request.getContextPath()%>/private/schedule/path?path=private.schedule.taskTypeEdit&taskType=-1"
	style="color: #0000CD">创建新任务...</a>
运行期信息：
<br />
...
<script>
	...
	function openDetail(obj, baseTaskType) {
		...
		obj.bgColor = "#FFD700";
		oldSelectRow = obj;
		document.getElementById("showTaskDetail").src = syschedule.contextPath + "/private/schedule/path?path=private.schedule.taskTypeInfo&baseTaskType=" + baseTaskType;
	}
	...
	function validateDel(str) {
		...
		if (flag) {
			window.location.href = syschedule.contextPath + "/private/schedule/path?path=private.schedule.taskTypeDeal&action=deleteTaskType&taskType=" + str;
		}
	}
</script>