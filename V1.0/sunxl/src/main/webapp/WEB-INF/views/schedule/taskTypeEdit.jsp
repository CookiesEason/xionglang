...
<form id="taskTypeForm" method="get" name="taskTypeForm"
	...
	<table>
		<tr>
			<td>任务名称:</td>
			<td>
				<input type="text" id="taskType" name="taskType" <%=editSts%>
					value="<%=taskType.getBaseTaskType()%>" width="30">
			</td>
			<td>任务处理的SpringBean:</td>
			<td>
				<input type="text" id="dealBean" name="dealBean" value="<%=taskType.getDealBeanName()%>"
					width="30">
			</td>
		</tr>
		<tr>
			<td>心跳频率(秒):</td>
			<td>
				<input type="text" name="heartBeatRate" value="<%=taskType.getHeartBeatRate()/1000.0 %>"
					width="30">
			</td>
			<td>假定服务死亡间隔(秒):</td>
			<td>
				<input type="text" name="judgeDeadInterval" value="<%=taskType.getJudgeDeadInterval()/1000.0 %>"
					width="30">
			</td>
		</tr>
		<tr>
			<td>线程数：</td>
			<td>
				<input type="text" name="threadNumber" value="<%=taskType.getThreadNumber()%>" width="30">
			</td>
			<td>处理模式：</td>
			<td>
				<input type="text" name="processType" value="<%=taskType.getProcessorType()%>" width="30">
				SLEEP 和 NOTSLEEP
			</td>
		</tr>
		<tr>
			<td>每次获取数据量：</td>
			<td>
				<input type="text" name="fetchNumber" value="<%=taskType.getFetchDataNumber() %>" width="30">
			</td>
			<td>每次执行数量：</td>
			<td>
				<input type="text" name="executeNumber" value="<%=taskType.getExecuteNumber() %>" width="30">
		...
		<tr>
			<td>没有数据时休眠时长(秒)：</td>
			<td>
				<input type="text" name="sleepTimeNoData" value="<%=taskType.getSleepTimeNoData()/1000.0%>"
					width="30">
			</td>
			<td>每次处理完数据后休眠时间(秒)：</td>
			<td>
				<input type="text" name="sleepTimeInterval" value="<%= taskType.getSleepTimeInterval()/1000.0%>"
					width="30">
			</td>
		</tr>
		<tr>
			<td>执行开始时间：</td>
			<td>
				<input type="text" name="permitRunStartTime"
					value="<%=taskType.getPermitRunStartTime()==null?"":taskType.getPermitRunStartTime()%>"
					width="30">
			</td>
			<td>执行结束时间：</td>
			<td>
				<input type="text" name="permitRunEndTime"
					value="<%=taskType.getPermitRunEndTime()==null?"":taskType.getPermitRunEndTime()%>" width="30">
			</td>
		</tr>
		<tr>
			<td>单线程组最大任务项：</td>
			<td>
				<input type="text" name="maxTaskItemsOfOneThreadGroup"
		...
		<tr>
			<td>自定义参数(字符串):</td>
			<td colspan="3">
				<input type="text" id="taskParameter" name="taskParameter"
		...
		<tr>
			<td>任务项(","分隔):</td>
			<td colspan="3">
				<TEXTAREA rows="5" id="taskItems" name="taskItems" style="width: 657"><%=taskItems%> </TEXTAREA>
			</td>
		</tr>
...
、[400,500)、[500,600)、[600,700)、[700,800)、[800,900)、 [900,1000)划分为十个任务项， 当然你也可以划分为100个任务项，最多是1000个项。
<br />
