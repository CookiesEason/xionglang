...
<form method="post" class="form" id="idImportDataInfoForm">
	<fieldset>
		...
		<table class="table" id="normortable">
			...
			<tr>
				...
				<td><select name="type" class="easyui-combobox">
						<c:forEach items="${sunxl_type}" var="sysData" varStatus="i">
							<option value="${sysData.value}" <c:if test='${sysData.intData==0}'>selected="selected"</c:if>>${sysData.paraOne}</option>
						</c:forEach>
				</select></td>
			</tr>
			<tr>
			...
			<tr>
				<th>摘要</th>
				<td colspan="3"><textarea name="smrytx" class="easyui-validatebox"
						style="height: 100px; width: 90%;" data-options="required:true"></textarea></td>
			</tr>
		</table>
	</fieldset>
</form>