...
<form method="post" class="form" id="idAdminUserForm">
	<fieldset>
		...
		<table class="table"
			...
			<tr>
				<th>菜单名<input type="hidden" value="${menu.id}" name="id" /></th>
			...
			<tr>
				...
				<td>
					<c:forEach items="${roles}" var="role" varStatus="i">
						<label><input type="checkbox" <c:forEach items="${menu.roles}" var="r" varStatus="j"><c:if test="${role.id==r.id}">checked="checked"</c:if></c:forEach> name="roles.id" value="${role.id}"><span>${role.name}</span></label>
					</c:forEach>
				</td>
			</tr>
		</table>
