...
<form method="post" class="form" id="idSequenceForm">
	<fieldset>
		...
		<table class="table"
			...
			<tr>
				<th>表名<input type="hidden" value="${idSequence.id}" name="id" /></th>
				<td><input value="${idSequence.tableName}" name="tableName"
					data-options="required:true" class="easyui-validatebox" /></td>
			</tr>
			<tr>
				<th>类路径</th>
				<td><input value="${idSequence.entityKey}" name="entityKey"
					data-options="required:true" class="easyui-validatebox" /></td>
			</tr>
			<tr>
				<th>主键值</th>
				<td><input value="${idSequence.entityValue}" name="entityValue"
					data-options="required:true" class="easyui-validatebox" /></td>
			</tr>
			<tr>
				<th>表用途</th>
				<td><textarea name="smrytx" class="easyui-validatebox"
						style="height: 100px; width: 90%;" data-options="required:true">${idSequence.smrytx}</textarea></td>
			</tr>
			<tr>
				<th>表创立时间</th>
				<td><input name="createTime" value="${idSequence.createTime}"
