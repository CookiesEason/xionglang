...
<form method="post" class="form" id="idImportDataColmForm">
	<fieldset>
		<legend>添加表字段</legend>
		<div>
			<input style="width: 80px; height: 35px; margin: 10px 0 10px 20px;" type='button'
				onclick="addrow()" value='新增字段' />
		</div>
		<table class="table" id="normortable">
			...
			<c:forEach items="${importDataColms}" var="colms" varStatus="i">
				<tr>
					<td>
						<input type="text" id="ordeid" name="importDataColm[${i.index}].ordeid" readonly="readonly"
							value="${colms.ordeid}" />
					</td>
					<td>
						<input type="text" id="coluna" onblur="checkColuna(this,${i.index+1})"
							name="importDataColm[${i.index}].coluna" value="${colms.coluna}" />
					</td>
					<td>
						<input type="text" id="defava" name="importDataColm[${i.index}].defava"
							value="${colms.defava}" />
					</td>
					<td>
						<input type="text" id="comment" name="importDataColm[${i.index}].comment"
							value="${colms.comment}" />
					</td>
					<td>
						<a href="javascript:gliImpUpRow(${i.index+1})" id="gliImpUpRow">上移</a>
						<a href="javascript:gliImpDownRow(${i.index+1})" id="gliImpDownRow">下移</a>
					</td>
					<td>
						<input type="button" class="button" value="删除" onclick="delImpColmRow(this,${i.index+1})" />
					</td>
				</tr>
			</c:forEach>
		</table>
