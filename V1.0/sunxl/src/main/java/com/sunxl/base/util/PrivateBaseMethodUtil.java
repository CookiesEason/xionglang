...
public class PrivateBaseMethodUtil {
	...
	public static String viewObject(HttpServletRequest request, GenericService genericService, String path, int id, String obj) {
		try {
			request.setAttribute(obj, genericService.find(genericService, id));
			request.setAttribute("result", "200");
			return path;
		} catch (Exception e) {
			logger.error("错误行号:【"+new Throwable().getStackTrace()[0].getLineNumber()+"】"+e.getMessage());
	 ...
	 * @此方法的作用：注意值的顺序values必须和colmnList的顺序相同，否则添加出位置不正确
	 */
	@MethodLog(remark = "批处理添加")
	public static boolean insertIntoTable(TableInfoBeanForm tbInfo, List<List<String>> values, List<ImportDataColm> colmnList) throws SQLException, ConnectionException {
		boolean flag = false;
		try {
