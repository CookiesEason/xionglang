...
public class DBSqlUtil {
	 ...
	 */
	public static <T> T getObject(Class<?> type, String sql, Object... params)
			throws ConnectionException, SQLException {
		T obj = null;
		ResultSet rs = null;
		PreparedStatement pst = null;
		try {
			rs = executeTypeSql(type, sql, pst, rs, params);
			if (rs.next()) {
				obj = (T) resultToClass(type, rs);
			}
		} catch (SQLException e) {
			throw new SQLException(e.getMessage());
		} finally {
	 ...
	 */
	public static <T> List<T> getObjectList(Class<?> type, String sql, Object... params)
			throws ConnectionException, SQLException {
	...
	private static ResultSet executeTypeSql(Class<?> type, String sql, PreparedStatement pst, ResultSet rs,
			Object... params) throws ConnectionException, SQLException {
		try {
			pst = SunxlConnectionFactory.getCurrentConnection().prepareStatement(sql);
			if (params != null) {
				int index = 1;
	...
	private static <T> T resultToClass(Class<?> type, ResultSet rs) {
		...
		String methodName = type.getName()
				.substring(type.getName().trim().lastIndexOf(".") + 1, type.getName().trim().length()).toUpperCase();
				...
				try {
					...
					if (methodName.startsWith("set")) {
						...
						if (types.length == 1) {
							...
							if (String.class.equals(types[0]) || char.class.equals(types[0])
	 ...
	 */
	private static String executeAddBatchObject(PreparedStatement pst, List<ImportDataColm> colmList,
			List<String> fields, TableInfoBeanForm tbInfo) throws SQLException {
		String exceptionMsg = "";
	 ...
	 * @auto：熊浪 @Time：2017年8月9日 @此方法的作用：
	 */
	private static void setStatement(PreparedStatement pst, String columnName, int idx, String val,
	 ...
	 * @此方法的作用：获取对应字段的数据类型
	 */
	public static String getColumnType(String columnName, TableInfoBeanForm tbInfo) throws SQLException {
	 ...
	 */
	public static TableInfoBeanForm getTableColumn(String tbName, TableInfoBeanForm tbInfo)
			throws SQLException, ConnectionException {
		if (tbInfo == null || (tbInfo != null && !tbInfo.getTableName().equals(tbName))
				|| (tbInfo != null && tbInfo.getColumnList().size() <= 0)) {
			try {
	 ...
	 * @此方法的作用：执行单挑数据预编译
	 */
	private static String executePrepareStatement(PreparedStatement pst, TableInfoBeanForm tbInfo, List<String> values,
	 ...
	 */
	private static boolean executeAllAddBatch(TableInfoBeanForm tbInfo, List<List<String>> values,
			List<ImportDataColm> colmList) throws SQLException, ConnectionException {
		boolean flag = false;
		PreparedStatement pst = null;
		if (colmList != null) {
			...
			pst = SunxlConnectionFactory.getCurrentConnection()
					.prepareStatement(getStringSql(tbInfo.getTableName(), 1, null, columns));
			try {
				if (values != null) {
	 ...
	 */
	public static boolean insertBatch(TableInfoBeanForm tbInfo, List<List<String>> values,
			List<ImportDataColm> colmnList) throws SQLException, ConnectionException {
		boolean flag = false;
		try {
			if (tbInfo != null && tbInfo.getColumnList() != null && tbInfo.getColumnList().size() > 0) {
				DBSqlUtil.executeAllAddBatch(tbInfo, values, colmnList);
				flag = true;
			}
		} catch (SQLException e) {
			logger.error("错误行号:【" + new Throwable().getStackTrace()[0].getLineNumber() + "】" + e.getMessage());
