...
public class Sql {
	 ...
	 * @auto：熊浪 @Time：2017年8月3日 @此方法的作用：
	 */
	public Sql addNull(String key) {
		if (offset >= params.length) {
			bulkCapacity(1);
		}
		params[offset++] = key + " IS NULL ";
		return this;
	}
	...
	public String getHqlFind() {
		sql = "SELECT o FROM " + clazz.getSimpleName() + " o " + build();
		logger.warn(sql);
		return sql;
	}

	public String getHqlUpdate() {
		sql = "UPDATE " + clazz.getSimpleName() + " o " + valueSet() + build();
		logger.warn(sql);
		return sql;
	}

	public String getHqlDelete() {
		sql = "DELETE FROM " + clazz.getSimpleName() + " o " + build();
		logger.warn(sql);
		return sql;
	}
	...
	/**
	 * @return
	 * @此方法的作用：规则跟创建HQL一样，但属性都改为字段名
	 */
	public String getSqlFind() {
		sql = "SELECT * FROM " + tableName + " o " + build();
		logger.warn(sql);
		return sql;
	}

	public String getSqlUpdate() {
		sql = "UPDATE " + tableName + " o " + valueSet() + build();
		logger.warn(sql);
		return sql;
	}

	public String getSqlDelete() {
		sql = "DELETE FROM " + tableName + build();
		logger.warn(sql.replace("o.", ""));
		return sql.replace("o.", "");
	}
}
